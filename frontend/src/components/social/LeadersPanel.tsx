              ...
              <div className="h-[calc(100%-72px)] overflow-y-auto px-[19px] py-[19px] space-y-[19px]">
                <div className="grid grid-cols-1 gap-6">
                  <div className="panel-base panel-orange p-6">
                    <div className="panel-caption text-left">Топ игроков</div>
                    <div className="rounded-xl bg-slate-950/25 overflow-hidden">
                      <div className="max-h-[60vh] overflow-auto">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {loading && (
                              <tr>
                                <td colSpan={3} className="px-4 py-6 text-center text-slate-300/70">
                                  Загрузка...
                                </td>
                              </tr>
                            )}
                            {!loading &&
                              topPlayers.map((player, index) => (
                                <tr key={player.id} className="border-t border-slate-600/25">
                            ...
                            {!loading &&
                              clans.map((clan) => {
                                const leader =
                                  ...
                                  clan.members?.[0];
                                return (
                                  <tr key={clan.id} className="border-t border-slate-600/25">
                                    <td className="px-4 py-3 text-slate-100">{clan.name}</td>
                                    <td className="px-4 py-3 text-slate-100">{leader?.username || '—'}</td>
                                    <td className="px-4 py-3 text-right text-slate-100">
