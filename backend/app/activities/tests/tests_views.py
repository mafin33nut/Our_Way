...
from app.activities.models import Activity, ActivityCategory

User = get_user_model()


class ActivitiesViewTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="apiuser", password="pass")
        self.client.login(username="apiuser", password="pass")
        self.cat = ActivityCategory.objects.create(name="Health")
    ...
    def test_start_stop_timer(self):
        act = Activity.objects.create(
            title="Direction session", owner=self.user, category=self.cat
        )
